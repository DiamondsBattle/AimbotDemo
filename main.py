...
def getAngleDif(f, t):
    ...
    f_y = f.camera_pivot.y - f.y
    v_dif = arctan((t.y - f_y + 2) / sqrt((t.z - f.z)**2 + (t.x - f.x)**2))
...
ground = Entity(model='cube', texture='white_cube', scale=Vec3(100, 1, 100), position=Vec3(2, -2, 2), collider='box', texture_scale=(2, 2, 1))
